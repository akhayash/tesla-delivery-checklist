...
test('重要度フィルタを切り替えると表示件数と保存内容が変わる', async ({
  ...
  await expect(page.getByTestId('checklist-filter-critical')).toContainText(/\(\d+\)/);
  await expect(page.getByTestId('checklist-filter-standard')).toContainText(/\(\d+\)/);
  await expect(page.getByTestId('checklist-filter-all')).toContainText(/\(\d+\)/);

  await expect(page.getByTestId('bulk-preview')).toContainText('表示中の 58 件');
  ...
  await expect(page.getByTestId('bulk-preview')).toContainText('表示中の 13 件');
