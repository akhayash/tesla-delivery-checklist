...
- **車種別チェックリスト**：JSON スキーマで構造化。現状は Model Y L 専用（6 人乗りキャプテンシート・3 列目アクセス・長尺パネルギャップ等の固有項目を含む 7 カテゴリ・65 項目）。
...
7 カテゴリ・65 項目（v0.1.0）。各項目に重要度（**重大 / 要対応 / 軽微**）を付与。

| カテゴリ | 項目数 | 内容 |
|---|---|---|
| 書類・基本情報 | 7 | VIN 一致、登録証、保証書、モバイルコネクター、キーカード × 2、フロアマット、取説 |
| 外装 / ボディ | 15 | パネルギャップ、ドアの建付け、塗装、ガラスルーフ、ライト、充電ポート、タイヤなど |
| 内装 (共通) | 9 | ステアリング、ダッシュボード、15.4" スクリーン、ドームライト、フランク、ラゲッジ、前席、オーディオ |
| 内装 (Model Y L 固有) | 10 | 2 列目キャプテンシート、ウォークスルー、3 列目アクセス・シート・ベルト、後席フラット収納、各列 USB-C、後席 HVAC |
| 機能 / 電装 | 15 | ワイパー、ウィンドウ、全席シートベルト、HVAC、音声認識ナビ、カメラ、Autopilot キャリブレーション、スーパーチャージャー試走、Sentry/Dog Mode 等 |
